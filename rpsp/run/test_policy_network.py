...
from policy_opt import policies
import rpsp.psr_lite.feat_extractor
import rpsp.psr_lite.gru_filter
import rpsp.psr_lite.rffpsr
import rpsp.psr_lite.rffpsr_rnn
import rpsp.psr_lite.rnn_filter
import rpsp.psr_lite_vrpg
import rpsp.psrlite_policy
from policy_opt.NN_policies import ContinuousExplorationPolicy, ContinuousPolicy
from envs.load_environments import load_environment
from explore.gaussian_strategy import GaussianStrategy
from filters.models import ObservableModel, FiniteHistoryModel
from policy_opt.nn_policy_updaters import VRPGPolicyUpdater
from policy_opt.policy_learn import learn_policy
from policy_opt.psr_policy_updaters import PSR_VRPGPolicyUpdater, PSR_AltOpt_TRPOPolicyUpdater
from test_utils.log import Log
from test_utils.plot import load_params, save_params
...
policy_updater = {
    ...
    #         {'lite-cont':rpsp.psr_lite_vrpg.VR_Reinforce_RNN_PolicyUpdater,
    #          'gru':rpsp.psr_lite_vrpg.VR_Reinforce_RNN_PolicyUpdater,
...
def load_rpsp_filter(args, data=[]):
    X_obs, X_act = data
    feats = rpsp.psr_lite.feat_extractor.create_RFFPCA_featureset(args.Hdim, args.dim, pw=args.kw, rng=args.rng)
    ...
    if args.reg is None:
        ...
        lambda_psr = rpsp.psr_lite.rffpsr.uniform_lambda(args.reg)
    psr = rpsp.psr_lite.rffpsr.RFFPSR(args.fut, args.past, args.dim, feature_set=feats, l2_lambda=lambda_psr,
    ...
    psr.freeze()
    psrrnn = rpsp.psr_lite.rffpsr_rnn.RFFPSR_RNN(psr, optimizer=args.roptimizer, optimizer_step=args.rstep,
...
def load_gru_filter(args, **kwargs):
    model = None
    rnngru = rpsp.psr_lite.gru_filter.GRUFilter(args.dim, args.dim, args.fut,
...
def rpsp_filter(args, data=[], **kwargs):
    ...
    if args.addobs:
        ...
        filter = rpsp.psr_lite.rnn_filter.ObsExtendedRNN(filter, kwargs['x_dim'], args.filter_w, args.mask_state)
...
#     feat_set = rpsp.rpsp.feat_extractor.create_RFFPCA_featureset(args.Hdim, args.dim, pw=args.kw, rng=args.rng)
#     model = ObservableModel(obs_dim=args.dim);
#
#     filter = rpsp.rpsp.rnn_filter.RFFobs_RNN(model, fset=feat_set, opt_U=args.wpca, opt_V=args.wrff,
#                                                        dim=args.dim)
...
def obs_model(args, data=[], **kwargs):
    ...
    filter = rpsp.psr_lite.rnn_filter.ObservableRNNFilter(model)
    filter.train(X_obs, X_act, on_unused_input='ignore')
    return model, filter
...
def load_rpsp_policy(args, model_exp, **kwargs):
    ...
    if isinstance(filter, rpsp.psr_lite.rffpsr_rnn.RFFPSR_RNN):
        pi = rpsp.psrlite_policy.RFFPSRNetworkPolicy(filter, pi_react, np.zeros((args.a_dim)))
    else:
        pi = rpsp.psrlite_policy.PSRLitePolicy(filter, pi_react, np.zeros((args.a_dim)))
