...
import rpsp.policy_opt.nn_policy_updaters as nn_policy_updaters
from rpsp.policy_opt.SGD_opt import adam
from rpsp.envs.environments import GymEnvironment
from rpsp.filters.models import ObservableModel
from rpsp.policy.policies import BasePolicy
from rpsp.policy_opt.policy_learn import BasePolicyUpdater
from rpsp.policy_opt.policy_learn import learn_policy
from rpsp.rpspnets.psr_lite.utils.regression import RidgeRegression
...
class DiscretePolicy(Policy):
    ...
    def sample_action(self, state):
        ...
        if math.isnan(prob[0]):
            raise "generated a nan probability..."
...
class ContinuousPolicy(Policy):
    def __init__(self, x_dim=None, output_dim=None, num_layers=None, nh=None, activation='relu', params=None, rng=None,
                 min_std=0.0):
        ...
        self.r = theano.shared(name='diag params',
                               value=-np.zeros(self.output_dim).astype(
                                   theano.config.floatX));  # this initliazation makes sure that the std (exp(r)) is around 0.3, so that 3-std is around 1, which is the max control input for mujoco setups.
...
class VR_Reinforce_PolicyUpdater(BasePolicyUpdater):  # using Rich Sutton's policy gradient theorem.
    ...
    def _transfer_trajs_to_matrixformat(self, trajs):
        ...
        for i in xrange(0, num_trajs):
            ...
            tmp_ctgs = np.array([-np.sum(trajs[i][3][j:] * self.gamma_seq[0:len(trajs[i][3][j:])]) for j in
                                 range(1, trajs[i][3].shape[0])]);
    ...
    def update(self, trajs):
        for i in xrange(len(trajs)):
            if self._policy.discrete is False:
                trajs[i].policy_grads = self._policy.compute_gradient_ll_wrt_state(states=trajs[i].states,
                                                                                   actions=trajs[i].act);
            elif self._policy.discrete is True:
                trajs[i].policy_grads = self._policy.compute_gradient_ll_wrt_state(states=trajs[i].states,
                                                                                   actions=trajs[i].act.reshape(
                                                                                       trajs[i].act.shape[0]));
    ...
    def __init__(self, x_dim, a_dim, output_dim, nh, LSTM=False, params=None, rng=None, ext=False):
        ...
        if LSTM is False:
            self.W_x = theano.shared(name='W_x', value=np.sqrt(2. / (self.x_dim + self.output_dim + self.nh))
                                                       * self.rng.randn(self.nh, self.x_dim + self.output_dim).astype(
            ...
            self.W = theano.shared(name='W', value=np.sqrt(2. / (self.output_dim + self.nh))
                                                   * self.rng.randn(self.output_dim, self.nh).astype(
                theano.config.floatX));
            ...
            self.W_i = theano.shared(name='W_i', value=np.sqrt(2. / (self.x_dim + self.output_dim + 2 * self.nh))
                                                       * self.rng.randn(self.nh,
                                                                        self.nh + self.x_dim + self.output_dim + self.ext_dim).astype(
                theano.config.floatX));
            ...
            self.W_c = theano.shared(name='W_c', value=np.sqrt(2. / (self.x_dim + self.output_dim + 2 * self.nh))
                                                       * self.rng.randn(self.nh,
                                                                        self.nh + self.x_dim + self.output_dim).astype(
                theano.config.floatX));
            ...
            self.W = theano.shared(name='W', value=np.sqrt(2. / (self.output_dim + 2 * self.nh))
                                                   * self.rng.randn(self.output_dim, 2 * self.nh).astype(
                theano.config.floatX));
            self.b = theano.shared(name='b',
                                   value=np.zeros(self.output_dim).astype(theano.config.floatX));
...
class RNN_Continuous_Policy(RNN_Policy):
    ...
    def update_mem(self, x):
        ...
        if self.mem is None:
            ...
            if np.isnan(self.mem).any():
                raise 'not nan h0 in update mem RNN policy'
        ...
        #        assert not
        if np.isnan(self.mem).any():
            raise 'not nan mem in update mem RNN policy'
        if self.LSTM is True:
            if np.isnan(self.cell).any():
                raise 'not nan cell in update mem RNN policy'
        return
    ...
    def _t_single_step_forward(self, x, a, h):
        ...
        h_tp1 = T.nnet.relu(T.dot(h, self.W_h.T) + T.dot(T.concatenate((x, a), axis=0), self.W_x.T) + self.hb,
                            alpha=0.01);
        return [h_tp1, y];  # return the mean.

    def _t_single_step_tradLSTM_forward(self, x, a, h, c):
        f = T.nnet.sigmoid(T.dot(T.concatenate((h, x, a), axis=0), self.W_f.T) + self.b_f)
        i = T.nnet.sigmoid(T.dot(T.concatenate((h, x, a), axis=0), self.W_i.T) + self.b_i)
        tilde_c = T.tanh(T.dot(T.concatenate((h, x, a), axis=0), self.W_c.T) + self.b_c)
        c_new = f * c + i * tilde_c
        o = T.nnet.sigmoid(T.dot(T.concatenate((h, x, a), axis=0), self.W_o.T) + self.b_o)
    ...
    def _t_single_step_extLSTM_forward(self, x, a, h, c):
        f = T.nnet.sigmoid(T.dot(T.concatenate((h, x, a, c), axis=0), self.W_f.T) + self.b_f)
        i = T.nnet.sigmoid(T.dot(T.concatenate((h, x, a, c), axis=0), self.W_i.T) + self.b_i)
        tilde_c = T.tanh(T.dot(T.concatenate((h, x, a), axis=0), self.W_c.T) + self.b_c);
        c_new = f * c + i * tilde_c
        o = T.nnet.sigmoid(T.dot(T.concatenate((h, x, a, c), axis=0), self.W_o.T) + self.b_o)
        h_new = o * T.tanh(c_new)
        y = T.dot(T.concatenate((h, c), axis=0), self.W.T) + self.b
        return [h_new, c_new, y]
...
class VR_Reinforce_RNN_PolicyUpdater(BasePolicyUpdater):
    ...
    def _padding(self, trajs):  # trajs: obs, state, u, reward.#U is in one-hot encoding.
        ...
        for i in xrange(0, self.num_trajs):
            ...
            tmp_ctgs = np.array([-np.sum(trajs[i][3][j:] * self.gamma_seq[0:len(trajs[i][3][j:])]) for j in
                                 range(0, trajs[i][3].shape[0])]);
            tensor_traj_ctg[i, 0:tmp_ctgs.shape[0]] = tmp_ctgs;
            trajs_tc.append(-np.sum(trajs[i][3]));  # convert reward to cost.

            if self.baseline is not False:
                self.states_set = trajs[i][1] if self.states_set is None else np.concatenate(
                    (self.states_set, trajs[i][1]), axis=0);
