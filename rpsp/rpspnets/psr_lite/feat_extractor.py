...
import rpsp.rpspnets.psr_lite.utils.kernel as kernel
import rpsp.rpspnets.psr_lite.utils.linalg as linalg
...
class RFFFeatureExtractor(FeatureExtractor):
    ...
    def _build(self, all_raw_data):                 
        ...
        if self._s is None:
            #self._s = kernel.median_bandwidth(all_raw_data, max=10000, rng = self._rng)
            self._s = kernel.precentile_bandwidth(all_raw_data, p=self._p, max=10000, rng=self._rng)
            
        self._V = kernel.sample_rff(self._num_samples, d, self._s, orth=self._orth, rng=self._rng)
        
    def _process(self, raw_data):        
        return kernel.rff(raw_data, self._V)
...
class RBFGramFunction(GramFunction):
    ...
    def build(self, all_raw_data):
        self._s = kernel.median_bandwidth(all_raw_data, self._p, rng=self._rng)
        
    def __call__(self,X,Y):
        return kernel.gram_matrix_rbf(X,Y,self._s)
...
class RBFDiagGramFunction(GramFunction):
    ...
    def build(self, all_raw_data):        
        self._s = kernel.median_diag_bandwidth(all_raw_data, max=1000, rng=self._rng)
        
    def __call__(self, X, Y):
        G = kernel.gram_matrix_diagrbf(X, Y, self._s)
        return G
...
class NystromFeatureExtractor(FeatureExtractor):
    ...
    def _build(self, all_raw_data):                    
        ...
        self._f = kernel.nystrom(all_raw_data, self._num_samples,
...
class ProjectionFeatureExtractor(FeatureWrapper):
    ...
    def _process(self, raw_data):          
        return linalg.blk_fn_row(lambda s,e: self._base_extractor.process(raw_data[s:e,:]).dot(self._U), raw_data.shape[0])
