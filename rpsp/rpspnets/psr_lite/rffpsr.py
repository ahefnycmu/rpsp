...
from rpsp.rpspnets.psr_lite.utils.nn import eval_CG, mia
from rpsp.rpspnets.psr_lite.utils.p3 import *
import rpsp.rpspnets.psr_lite.utils.linalg as ula
import rpsp.rpspnets.psr_lite.utils.opt as opt
import rpsp.rpspnets.psr_lite.utils.feats as ufeats
import rpsp.rpspnets.psr_lite.feat_extractor as feat_extractor
from rpsp.rpspnets.psr_lite.utils.regression import ridge
from rpsp.rpspnets.psr_lite.psr_base import ControlledModel, UncontrolledModel, extract_timewins, structtype
...
class RFFPSR(ControlledModel):
    ...
    def _compute_importance_weights(self, data, feats, traj_act_probs):                        
        if traj_act_probs is None:
            ...
            fut_extractor = ufeats.finite_future_feat_extractor(self._fut)
            extended_extractor = ufeats.finite_future_feat_extractor(self._fut+1)
            ...
            prob_future = np.prod(ufeats.flatten_features(traj_act_probs, fut_extractor, bounds)[0], 0)
            prob_extended = np.prod(ufeats.flatten_features(traj_act_probs, extended_extractor, bounds)[0], 0)
