...
def row_kr_product(X,Y,name=None):
    ...
    if X.ndim == 1 and Y.ndim == 1:
        ...
        output = (X.reshape((X.shape[0],X.shape[1],1)) * Y.reshape((Y.shape[0],1,Y.shape[1]))).reshape((X.shape[0],X.shape[1]*Y.shape[1]))
...
def tf_get_normalized_grad_per_param(loss, params, beta=0.1, normalize=True, clip_bounds=[]):
    ...
    #grad = T.grad(cost=loss, wrt=params, disconnected_inputs='ignore' ) 
    updates=[]; vars_new = T.ones([]); grad = [];
    powerg = globalconfig.vars.args.powerg #2
    squashg = globalconfig.vars.args.squashg
...
def t_clip_percentile(msg,X,threshold=0.9):       
    def fn(x): 
        ...
        print(msg+'#>{}= {}/{}, sum>{}={}, sum<{}={}'.format(threshold,np.sum(x>pval), np.prod(x.shape),threshold,np.sum(x*(x>pval)),threshold,np.sum(x*(x<=pval)) ))
        x = x*(x<pval)
        return x                            
...
def cg_solve_batch(AA, B, iter=10, reg=0.0, use_scan=False):
    ...
    if use_scan:
        result, _ = theano.scan(fn=_cg_step_batch, outputs_info=[X0,R0,R0], non_sequences=[AA,reg], n_steps=iter, strict=True)
        return result[0][-1]
    else:
        Xi = X0
        Ri = R0
...
class TheanoSGDOptimizer(TheanoOptimizer):
    ...
    def init(self, output_tf, inputs, params, on_unused_input='raise'):
        ...
        self._fn = theano.function(inputs=inputs + [step_size], outputs=output_tf, updates=updates, on_unused_input=on_unused_input)        
...
def CGD_optimizer(As,bs, iter=10, C=None):
    ...
    r0 = b-T.dot(A,x0)
    z0 = T.dot(C,r0)
    result, updates = theano.scan(fn=CG_single_step , outputs_info=[x0,r0,z0], non_sequences=[A,C], n_steps=iter, strict=True )
    return result[0][-1]
...
def test_CG():
    ...
    tic = time.time();  yy_hat = eval(A,b); t2 = time.time()-tic;
    tic1 = time.time(); ny_hat = CGD_num_optimizer(A, b, iter=NITER); t3 = time.time()-tic1;
    tic2 = time.time(); y_hat = TheanoCGDOptimizer(inputs=(TA,Tb)).minimize(iter=NITER).get_value(); t1 = time.time()-tic2;
