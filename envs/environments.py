...
import numpy as np
from models import Trajectory, ObservableModel
import time
    ...
    def run(self, model, policy, max_traj_length, min_traj_length=0,
        ...
        while not done_all:                        
            ...
            for j in xrange(max_traj_length):  
                ...
                forward_pos = self.env_state[0][0] #qpos[0]
            ...
            if j >= min_traj_length:
                ...
                if not drop_traj:                    
                    ...
                    new_traj = Trajectory(obs=obs[:j,:],states=states[:j,:],act=act[:j,:],rewards=rwd[:j,:],
                                          act_probs=act_probs[:j,:], obs0=o0, state0=q0, rng = rngs[-1], vel=vel[:j,:])#, env_states=env_states)
                    ...
                    if num_trajs > 0 and len(trajs) == num_trajs:
                        ...
                        done_all = True     
                    #if len(trajs) % 100 == 0:
                    #    print 'Generating Trajectories: %d trajectories with %d samples' % (len(trajs), i_sample)
...
class GymEnvironment(Environment):
    ...
    def __init__(self, gym_env, discrete = True, rng=None):
        ...
        #if isinstance(gym_env,gym.wrappers.time_limit.TimeLimit):
        #    print('gym.env')
        #    gym_env = gym_env.env
        #elif isinstance(gym_env.env,gym.wrappers.time_limit.TimeLimit):
        #    print('gym.env.env')
        #    gym_env = gym_env.env.env
...
class RLEnvironment(Environment):
    '''
    Create an Envinroment wrapper on top of an RLPy environment
    '''
    def __init__(self, gym_env, discrete=False, rng=0):
        self.env = gym_env(discrete=discrete,rng=rng)
        self.discrete = discrete;
    ...
    def dimensions(self):
        if self.discrete is True:
            return (self.env.statespace_limits.shape[0],1)
        else:
            return (self.env.statespace_limits.shape[0],
                    self.env.d-1)
    ...
    def reset(self):
        o,done,_ = self.env.s0()
        return o
        
    def step(self, a):
        r,o,done,_ = self.env.step(a)
        return (o,r,done)

    def render(self):
        self.env.showDomain(None)  
...
class RandomStopEnvironment(EnvironmentWrapper):
    '''
    A wrapper to make a base environment stop after a random number of steps 
    within a specified range.
    '''
    def __init__(self, base_environment, min_length, max_length, rng=None):
        EnvironmentWrapper.__init__(self, base_environment)
    ...
    def reset(self):
        self._length = self._rng.randint(self._min_length,self._max_length+1)
        self._t = 0
        return self._base.reset()
...
class ContinuousEnvironment(GymEnvironment):
    '''
    create wrapper for continuous environment pass the continuous simulator. 
    '''
    def __init__(self, env, simulator, rng=None):
        self.sim = simulator
        super(ContinuousEnvironment,self).__init__( env, discrete=False, rng=rng)
        
    @property
    def dt(self):
        return self.sim.dt(self.env)
