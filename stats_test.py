...
from test_crossent_models import test_crossentropy
...
functions={ 'obsCE': test_crossentropy,
            'lite-contCE': test_crossentropy,
            'lite-obsCE': test_crossentropy,
            'arCE':  test_crossentropy,
            'deltaArCE':  test_crossentropy,
            'lstmCE':  test_crossentropy,
           }
...
def run_Nmodel( args, filename, N=5, plot=True, loadfile=''):
    try:
        ...
        for i in xrange(N):
            ...
            functions.get(args.method,test_policy_continuous)(args, filename)
        return
    except IOError:
        ...
        for i in xrange(N):
            ...
            args.trial = i
            results = functions.get(args.method,test_policy_continuous)(args, filename)
...
def run_model( args, filename, plot=True, loadfile=''):
    
    try:
        print 'Loading existing model...'
        load_file = (filename,loadfile)[loadfile<>'']
        results, nargs = load_model(args.method+'_t%d.pkl'%args.trial, load_file)
        print 'trial %d'%(args.trial)
        if args.monitor is not None:
            args.monitor+='_t%d'%args.trial
        setattr(args, 'params', results['params'])
        functions.get(args.method,test_policy_continuous)(args, filename)
        return
    except IOError:
        setattr(args,'loadfile','')
        print 'Model non-existent. Running experiment:'
        print 'trial %d'%args.trial, args.iter
        results = functions.get(args.method,test_policy_continuous)(args, filename)
        if results is not None:
            save_model(args.method+'_t%d'%args.trial, filename, results, args)
            if plot:
                plot_trials([results], filename+'t%d_'%args.trial)
...
if __name__ == '__main__':
    ...
    best_models={'Hopper':{'random_PSR':('random_PSR',mcolors.cnames['black'],symb4),'fix_PSR':('Alt_fix_PSR',mcolors.cnames['brown'],symb2),'reactive_PSR':('Alt_reactive_loss',mcolors.cnames['grey'],symb3),'VRPG':('VRPG+obs',mcolors.cnames['dodgerblue'], symb1),'Alt':('Alt',mcolors.cnames['limegreen'],symb1),'RPSP':('Alt',mcolors.cnames['dodgerblue'],symb1), 'VRPG-GRU':('VRPG-gru+obs',mcolors.cnames['darkblue'],symb3), 'Alt-GRU':('Alt-gru+obs',mcolors.cnames['darkgreen'],symb3), 'reg_GRU':('Alt-gru+obs',mcolors.cnames['orange'],symb3),'GRU':('GRU',mcolors.cnames['red'],symb3),'reactive_GRU':('react_GRU',mcolors.cnames['purple'],symb3),'PSD':('PSD',mcolors.cnames['brown'],symb3)},
                 'Swimmer':{'random_PSR':('random_PSR',mcolors.cnames['black'],symb4),'fix_PSR':('VRPG_fix_PSR',mcolors.cnames['brown'],symb2),'reactive_PSR':('VRPG_reactive_loss',mcolors.cnames['grey'],symb3),'VRPG':('VRPG+obs',mcolors.cnames['dodgerblue'], symb1),'Alt':('Alt+obs',mcolors.cnames['limegreen'],symb1),'RPSP':('VRPG+obs',mcolors.cnames['dodgerblue'],symb1),'VRPG-GRU':('VRPG-gru+obs',mcolors.cnames['darkblue'],symb3), 'Alt-GRU':('Alt-gru+obs',mcolors.cnames['darkgreen'],symb3), 'reg_GRU':('Alt-gru+obs',mcolors.cnames['orange'],symb3),'GRU':('GRU',mcolors.cnames['red'],symb3),'reactive_GRU':('react_GRU',mcolors.cnames['purple'],symb3),'PSD':('PSD',mcolors.cnames['brown'],symb3)},
                 'CartPole':{'random_PSR':('random_PSR',mcolors.cnames['black'],symb4),'fix_PSR':('Alt_fix_PSR',mcolors.cnames['brown'],symb2),'reactive_PSR':('Alt_reactive_loss',mcolors.cnames['grey'],symb3),'VRPG':('VRPG',mcolors.cnames['dodgerblue'], symb1),'Alt':('Alt',mcolors.cnames['limegreen'],symb1),'RPSP':('VRPG',mcolors.cnames['dodgerblue'],symb1),'VRPG-GRU':('VRPG-gru+obs',mcolors.cnames['darkblue'],symb3), 'Alt-GRU':('Alt-gru+obs',mcolors.cnames['darkgreen'],symb3), 'reg_GRU':('Alt-gru+obs',mcolors.cnames['orange'],symb3),'GRU':('GRU',mcolors.cnames['red'],symb3),'reactive_GRU':('react_GRU',mcolors.cnames['purple'],symb3),'PSD':('PSD',mcolors.cnames['brown'],symb3)},
                 'Walker2d':{'random_PSR':('random_PSR',mcolors.cnames['black'],symb4),'fix_PSR':('Alt_fix_PSR',mcolors.cnames['brown'],symb2),'reactive_PSR':('Alt_reactive_loss',mcolors.cnames['grey'],symb3),'VRPG':('VRPG+obs',mcolors.cnames['dodgerblue'], symb1),'Alt':('Alt+obs',mcolors.cnames['limegreen'],symb1),'RPSP':('Alt+obs',mcolors.cnames['dodgerblue'],symb1),'VRPG-GRU':('VRPG-gru+obs',mcolors.cnames['darkblue'],symb3), 'Alt-GRU':('Alt-gru+obs',mcolors.cnames['darkgreen'],symb3), 'reg_GRU':('Alt-gru+obs',mcolors.cnames['orange'],symb3),'GRU':('GRU',mcolors.cnames['red'],symb3),'reactive_GRU':('react_GRU',mcolors.cnames['purple'],symb3),'PSD':('PSD',mcolors.cnames['brown'],symb3)},
                 }
    ...
    for i in xrange(len(envs)):
        ...
        if False and env=='Hopper':
            ...
            fig = plot_trials(vel_new, fdir+envs[i]+'-v1/', f=lambda it: vel_new[it], name='Hopper_opt', shape='-s', step=20)
            plt.ylabel(envs[i]+' Cumulative reward $R_{iter}$')
            fig = plot_trials(vel_old, fdir+envs[i]+'-v1/', f=lambda it: vel_old[it], name='Hopper_Gym',fig=fig,shape='-o',step=20)
        ...
        if True:
            ...
            for j,noise in enumerate(noise_levels):
                R_iter, R_cum, auc = plot_rwds( noise_labels, cdict=color_dict, fdir=fdir+envs[i]+'-v1/'+fdir_add, \
                  model_paths=noise_paths(noise), BEST=BEST, MAX_LEN=MAX_LEN(envs[i]), STEP=STEP, MAX_TRIALS=MAX_TRIALS,
            ...
            f.savefig(fdir+envs[i]+'-v1/'+envs[i]+'noiseobs_rwdperiter_best%d_step%d.png'%(BEST,STEP), dpi=300)
            #print 'donenoise'
        ...
        if False:
            ...
            plot_rwds( RPSPgru_labels, cdict=best_colors(envs[i]), fdir=fdir+envs[i]+'-v1/'+fdir_add, \
                          model_paths=best_paths(envs[i]), BEST=BEST, MAX_LEN=MAX_LEN(envs[i]), STEP=STEP, MAX_TRIALS=MAX_TRIALS,
                          title=envs[i]+' GRU Filter comparison $R_{iter}$ ', step = max_lims[envs[i]][1], shape_dict=best_shape(envs[i]),
                          kfold=max_lims[envs[i]][1]*MAX_TRIALS)
            
        if True:
            '''Experiments for RPSP/gru minimal comparison'''
            x, x, aucurve = plot_rwds( filter_labels, cdict=best_colors(envs[i]), fdir=fdir+envs[i]+'-v1/'+fdir_add, \
                          model_paths=best_paths(envs[i]), BEST=BEST, MAX_LEN=MAX_LEN(envs[i]), STEP=STEP, MAX_TRIALS=MAX_TRIALS,
                          title=envs[i]+' PSR vs. GRU filtering $R_{iter}$ ', step = max_lims[envs[i]][1], shape_dict=best_shape(envs[i]), ylim=[-10,max_lims[envs[i]][0]],
        ...
        if True:
            x, x, aucurve = plot_rwds( reg_labels, cdict=best_colors(envs[i]), fdir=fdir+envs[i]+'-v1/'+fdir_add, \
                          model_paths=best_paths(envs[i]), BEST=BEST, MAX_LEN=MAX_LEN(envs[i]), STEP=STEP, MAX_TRIALS=MAX_TRIALS,
                          title=envs[i]+' Filter regularization $R_{iter}$ ', step = max_lims[envs[i]][1], shape_dict=best_shape(envs[i]), ylim=[-10,max_lims[envs[i]][0]],
                          kfold=max_lims[envs[i]][1]*MAX_TRIALS)
        ...
        if True:
            ...
            print env
            bestmodels, R_cum, aucurve = plot_rwds( envs_labels, cdict=color_dict, fdir=fdir+envs[i]+'-v1/'+fdir_add, \
                  model_paths=model_paths, BEST=BEST, MAX_LEN=MAX_LEN(envs[i]), STEP=STEP, MAX_TRIALS=MAX_TRIALS,
                  title=envs[i]+' $R_{iter}$ ', step = max_lims[envs[i]][1],shape_dict=shape_dict,
                  kfold=max_lims[envs[i]][1]*MAX_TRIALS)
            print 'done print'
...
if False:
    ...
    ncolor_dict = {}
    ncolor_dict.update(dict([(k,colors_noise1[-1]) for (i,k) in enumerate(list(filter(lambda k: k[:4]=='join', noise_paths.keys()))) ]))
    ncolor_dict.update(dict([(k,colors_noise2[-1]) for (i,k) in enumerate(list(filter(lambda k: k[:3]=='obs', noise_paths.keys())))  ]))
    ncolor_dict.update(dict([(k,colors_noise3[-1]) for (i,k) in enumerate(list(filter(lambda k: k[:2]=='AR', noise_paths.keys())))   ]))
