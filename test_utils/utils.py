...
def render_trajectory(env, env_states, render=True, speedup=0.1):
    ...
    for j in xrange(len(env_states)-1):  
        if render:
            env.render()
            timestep = 0.05
...
def play_video_trajs(fdir, fname, save=False, play=True, niter=-1):
    ...
    from stats_test import load_model
    from models import Trajectory
...
if __name__ == '__main__':
    ...
    play_best_video_traj(fdir,fna   me)