#import PSIM_backpropagation as psim_bp
import numpy as np
...
import sys
from psr_lite.psr_lite.utils.nn import dbg_print_stats, dbg_print, dbg_nn_assert_notnan, dbg_nn_assert_notinf
...
#Xs is 3D tensor: Num_of_steps * dx * Num_of_trajs. 
...
def grad_dbug(grad_update_func, *args,**kwargs):
    grads = args[0]
    params = args[1]
    for i in xrange(len(grads)):
        #grads[i] = dbg_nn_assert_notnan(grads[i], 'grad_%s is NAN !!'%params[i].name)
        #grads[i] = dbg_nn_assert_notinf(grads[i], 'grad_%s is INF !!'%params[i].name)
        grads[i] = dbg_print_stats( 'grad_%s: '%params[i].name, grads[i])
        #params[i] = dbg_print_stats( 'param_%s: '%params[i].name, params[i])
    args = list(args)
    args[0] = grads
    args[1] = params
    return grad_update_func(*args,**kwargs)





def natural_sgd(loss, all_params, nabla, curr_params, learning_rate = 0.01, epsilon = 1e-3): #nabla here is nXd. 
	updates = [];
...
def SGD_train(Xs_train, Xs_test, kf, step_ahead = 1, learning_rate = 0.1,  n_epochs = 20, nh = 100, method = 'PSIM', optimizer = 'sgd',
	...
	if method == 'PSIM':
		if W_initial is None:
			...
			model = psim_bp.PSIM_BackPropagation(kf, dx, m_0, W_x = W_initial[:,0:dx], W_mess = W_initial[:,dx:], b = np.zeros(m_0.shape[0]));
	...
	for e in range(0, n_epochs): 
		...
		if abs(be - e) > rate_decay_step: #if no improvement found among the last 5 epoch, we decay the learning rate half. 
			print "No improvement found on the validation set in the last {} epoch, decay learning rate from {} to {}".format(rate_decay_step, 
...
def simple_SGD_train(Xs_train, Xs_test, kf, step_ahead = 1, learning_rate = 0.1,  n_epochs = 20, nh = 100, method = 'PSIM', optimizer = 'sgd',
				rate_decay_step = 5, learning_rate_threshold = 1e-5, W_initial = None, rng=None):
